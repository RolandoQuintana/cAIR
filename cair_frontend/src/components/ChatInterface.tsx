...
export function ChatInterface({ storyId }: ChatInterfaceProps) {
  ...
  const renderMessage = (message: Message) => {
    ...
    return (
      ...
      >
        <div className={`max-w-[85%] sm:max-w-xs lg:max-w-md xl:max-w-lg ${isUser ? 'order-2' : 'order-1'}`}>
          <div
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 sm:space-y-4">
        ...
        {chatState.loading && chatState.messages.length === 0 && (
          <div className="text-center py-6 sm:py-8">
            ...
            <p className="mt-2 text-sm sm:text-base text-gray-600">Loading conversation...</p>
          </div>
        )}
        ...
        <form onSubmit={handleSendMessage} className="flex space-x-2 sm:space-x-4">
          <div className="flex-1">
            <textarea
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none min-h-[40px] max-h-32 text-sm sm:text-base"
              disabled={isSending}
              rows={1}
            />
          </div>
          <button
            ...
            className="px-3 py-2 sm:px-4 sm:py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center min-w-[44px] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
          >
            {isSending ? (
